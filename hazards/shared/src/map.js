        ...
        if (clickPt.x === evt.mapPoint.x && clickPt.y === evt.mapPoint.y) {
            ...
            dblClickTimer = setTimeout(function(){
                if (!doubleClicked) {
                    ...
                    if (globals.srid === 3572) { //Arctic projection
...
function normalizeAndPublishArcticExtent(extent, symbol) {
    ...
    var densifiedPoly = densifyExtent(extent, 10000);
 ...
 */
function densifyExtent(extent, distance) {
    var points = [];
    var x, y;
    ...
    var polygon = new esri.geometry.Polygon(new esri.SpatialReference({wkid: 3572}));
