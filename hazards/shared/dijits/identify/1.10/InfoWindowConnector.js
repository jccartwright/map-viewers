dojo.provide("identify.InfoWindowConnector");

dojo.require("dojo.dnd.Mover");
dojo.require("dojo.dnd.Moveable");
dojo.require("dojo.dnd.move");

dojo.declare("identify.InfoWindowConnector", null, {
  constructor: function(/*DOMNode | String*/ domNode) {
    ...
    this._visible = true;
    this._moveableHandleTop = null;
	this._moveableHandleBottom = null;
	this._moveableHandleLeft = null;
	this._moveableHandleRight = null;
  ...
  attachToMap: function(/*esri.Map*/ map) {
    ...
    if (!this._attached) {
      ...
      if (this._moveableHandleTop) {
        this._moveableHandleTop.destroy();
      }
	  if (this._moveableHandleBottom) {
        this._moveableHandleBottom.destroy();
      }	
	  if (this._moveableHandleLeft) {
        this._moveableHandleLeft.destroy();
      }
	  if (this._moveableHandleRight) {
        this._moveableHandleRight.destroy();
      }	  
      this._attached = true;
  ...
  detachFromMap: function(/*String?*/ handleId) {
    ...
    this._moveableHandleTop = new dojo.dnd.move.parentConstrainedMoveable(this._node, { area: "content", within: true, handle: handleId });
    this._moveableHandleBottom = new dojo.dnd.move.parentConstrainedMoveable(this._node, { area: "content", within: true, handle: this._node.id + "-bottombar" });
    this._moveableHandleLeft = new dojo.dnd.move.parentConstrainedMoveable(this._node, { area: "content", within: true, handle: this._node.id + "-leftbar" });
    this._moveableHandleRight = new dojo.dnd.move.parentConstrainedMoveable(this._node, { area: "content", within: true, handle: this._node.id + "-rightbar" });
  ...
  show: function(/*esri.geometry*/ geometry) {
  	if (geometry instanceof esri.geometry.Extent) {
		this._mapPoint = new esri.geometry.Point(geometry.xmax, geometry.ymin, new esri.SpatialReference({ wkid: 3857 }));
	}
	else if (geometry instanceof esri.geometry.Point) {
		this._mapPoint = geometry;	
	}
	else if (geometry instanceof esri.geometry.Polygon) {
		this._mapPoint = new esri.geometry.Point(geometry.getExtent().xmax, geometry.getExtent().ymin, new esri.SpatialReference({ wkid: 3857 }));
	}
	else {
		console.log("InfoWindowConnector: geometry type not supported");
		return;
	}
