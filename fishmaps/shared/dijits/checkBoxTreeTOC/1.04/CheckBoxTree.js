dojo.provide("checkBoxTreeTOC.CheckBox");
dojo.provide("checkBoxTreeTOC.CheckBoxTree");
dojo.provide("checkBoxTreeTOC.CheckBoxStoreModel");

// THIS WIDGET IS BASED ON DOJO/DIJIT 1.4.0 AND WILL NOT WORK WITH PREVIOUS VERSIONS
//
//	Release date: 02/12/2010
//
// Adapted from code found here: http://www.thejekels.com/blog/dojo/dijit-tree-with-multi-state-checkboxes/
...
/*
(function(){
	var css = [ dojo.moduleUrl(
		"checkBoxTreeTOC", "themes/tundra/Checkbox.css") // custom css used by this dijit: you can customize this file to change the look and feel of this dijit
	];
...
dojo.declare( "checkBoxTreeTOC.CheckBox", dijit.form.CheckBox,
{
	// baseClass: [protected] String
	//		Root CSS class of the widget (ex: pdirCheckBox), used to add CSS classes of widget
...
dojo.declare( "checkBoxTreeTOC.CheckBoxStoreModel", dijit.tree.ForestStoreModel, 
{
	// checkboxAll: Boolean
	...
	updateCheckbox: function(/*dojo.data.Item*/ storeItem, /*Boolean*/ newState ) {
		...
		//
		//console.log("inside updateCheckbox... " + storeItem.name + " " + newState);
		if( !this.checkboxStrict ) {
			this._setCheckboxState( storeItem, newState, false, true );		// Just update the checkbox state
		} 
		else {
			this._updateChildCheckbox( storeItem, newState, true );			// Update children and parent(s).
		}
	},
		...
		if ( storeItem == this.root ) {
			if( typeof(storeItem.checkbox) == "undefined" ) {
				...
				if( this.checkboxRoot ) {
					this._setCheckboxState( storeItem, this.checkboxState, false, true );
	...
	_setCheckboxState: function(/*dojo.data.Item*/ storeItem, /*Boolean*/ newState, /*Boolean*/ mixedState, /*Boolean*/ publish ) {
		// summary:
		...
		//  publish:
		//		Should the new layer visibility be published to the main web app? Should be false for all children when toggling "folders"
		//	example:
		//	| model.setCheckboxState(item, true, false, true);
		//
		//console.log("inside setCheckboxState " + storeItem.name + " " + newState + " " + mixedState + " " + publish)
		...
		//Publish layer visibility to main web app
		if (publish && storeItem.leaf == 'true') {
			//console.log("publish: " + storeItem.service + " " + storeItem.subLayers + " " + newState);
			dojo.publish("/toc/layer/show", [{
				name: storeItem.name,
				service: storeItem.service[0],
				subLayers: storeItem.subLayers,
				state: newState
			}]);
		}		
		return stateChanged;
	...
	_updateChildCheckbox: function(/*dojo.data.Item*/ storeItem, /*Boolean*/ newState, /*Boolean*/ publish ) {
		//	summary:
		...
		//  publish:
		//		Should the new layer visibility be published to the main web app? Should be false for all children when toggling "folders"
		//console.log("inside updateChildCheckbox... " + storeItem.name + " " + newState + " " + publish);
		if( this.mayHaveChildren( storeItem )) {
			var childServices = [];
			var childSubLayers = [];
			
			//First update the child checkboxes, then publish the new sublayer visibilities to the main web app.
			//One dojo.publish event will be sent per map service.
			
			this.getChildren( storeItem, dojo.hitch( this,
				function( children ) {
					dojo.forEach( children, function(child) {
						var service = child.service[0];
						
						//Populate an array of which map services are about to be updated, without duplicates
						var idx = dojo.indexOf(childServices, service);
						if (idx == -1) {
							childServices.push(service);
							if (!childSubLayers[service]) {
								childSubLayers[service] = [];
							}
						}
						
						dojo.forEach(child.subLayers, function(subLayer) {																												
							//Populate an associative array of which sublayers are about to be toggled for each map service							
							childSubLayers[service].push(subLayer);							
						}, this);						
						this._updateChildCheckbox( child, newState, false );
					}, this );					
				}),
				function(err) {
			...
			//Walk through the services and publish sublayer visiblity to main web app
			dojo.forEach(childServices, function(childService) {
				//console.log("publish: " + childService + " " + childSubLayers[childService] + " " + newState);
				dojo.publish("/toc/layer/show", [{
					name: storeItem.name,
					service: childService,
					subLayers: childSubLayers[childService],
					state: newState
				}]);	
			}, this);
			...
			if( this._setCheckboxState(storeItem, newState, false, publish) ) {
		...
		dojo.forEach( parents, function( parentItem ) {
			this.getChildren( parentItem, dojo.hitch( this,
				function(children) {
					...
					if( this._setCheckboxState( parentItem,	isMixed ? true: hasChecked, isMixed ? true: false, true ) ) {
...
dojo.declare( "checkBoxTreeTOC._CheckBoxTreeNode", dijit._TreeNode,
{
	// _checkbox: [protected] dojo.doc.element
	//		Local reference to the dojo.doc.element of type 'checkbox'
	_checkbox: null,
	templateString: dojo.cache('checkBoxTreeTOC', 'templates/TreeNode.html'),
	...
	_createCheckbox: function() {
		...
		if( state.checked !== undefined ) {
			if (this.tree.checkboxStyle == "dijit" ) {
				this._checkbox = new checkBoxTreeTOC.CheckBox().placeAt(this._checkboxCell);				
			} else {
			...
			//Publish layer visibility to main web app
			if (this.item.leaf == 'true') {
				//console.log("publish /toc/layer/show: " + this.item.service + " " + this.item.subLayers + " " + this.item.checkbox);
				dojo.publish("/toc/layer/show", [{
					name: this.item.name,
					service: this.item.service[0],
					subLayers: this.item.subLayers,
					state: this.item.checkbox == 'true' ? true : false 
				}]);
			}		
		}
		...
		//Get rid of the treeExpand_leaf icon for top-level items without children
		if ( this.item.type == 'item' && this.item.children == null) {
			dojo.removeClass(this.expandoNode,"dijitTreeExpando");		
		}			
	},
...
dojo.declare( "checkBoxTreeTOC.CheckBoxTree", dijit.Tree,
{
	...
	postCreate: function() {
		...
		if(!store.getFeatures()['dojo.data.api.Write']){
			throw new Error("checkBoxTreeTOC.CheckboxTree: store must support dojo.data.Write");
		}
	...
	_createTreeNode: function( args ) {
		...
		// description:
		//		Create a new CheckboxTreeNode instance.
		return new checkBoxTreeTOC._CheckBoxTreeNode(args);
	},
