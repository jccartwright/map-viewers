...
dojo.declare("identify.Identify", [dijit._Widget, dijit._Templated], {
	...
	constructor: function(params, srcNodeRef){
		...
		this._featureRowTemplate = 
			"<span class='genid-featurename'>${value}</span>" +
			"<span id='${spanId}' class='genid-grid-row'><a id='${linkId}' href='#' class='genid-focus-link'><img src='" + dojo.moduleUrl("identify", "images/magnifying-glass.png").toString() + "'/></a></span>" +
			"<br/><span class='genid-layer-name'>${layerName}</span>";
		//this._attributeRowTemplate = "${value}<br/><span class='genid-attr-name'>${attr}</span>";
		this._attributeRowTemplate = '<table><tr><td valign="top" width="100"><b>${attr}:</b></td><td valign="top">${value}</td></tr></table>';
		...
		this._getDataButtonLabel = "Get Data";
		this._showSummaryOrBackButtonLabel = "Show Summary";
		this._showNewWindowButtonLabel = "Show In New Window";
	...
	startup: function(){
		...
		*/
				...
				if (this.mapServices[i].name == params.serviceId) {																																																						
					...
					for (var j = 0; j < params.visibleLayers.length; j++) {
						...
						if (typeof this.mapServices[i].displayOptions[params.visibleLayers[j]] == "undefined" ) {
							var idx = dojo.indexOf(this.mapServices[i].layerIds, params.visibleLayers[j]);
							this.mapServices[i].layerIds.splice(idx, 1);  					
		...
		dojo.subscribe('/ngdc/mapClick', this, function(geometry) {
			//console.log('/ngdc/mapClick');		
	...
	_executeIdentify: function(geometry) {		
		...
		for (i = 0; i < services.length; i++) {
			...
			if (service.service instanceof esri.layers.ArcGISDynamicMapServiceLayer || service.service instanceof layers.PairedMapServiceLayer) {
				params.layerIds = service.service.visibleLayers;
			}
			...
			params.layerDefinitions = service.service.layerDefinitions;
			...
			function(){
				...
				var maxRecordCount = service.service.maxRecordCount
				...
				return function(response){
					...
					var isCapped = false;
					if (response.length >= maxRecordCount) {
						isCapped = true;
					}

					this._showFeatures(response, geometry, options, serviceUrl, sortFunction, isCapped);	
	...
	_showFeatures: function(idResults, geometry, options, serviceUrl, sortFunction, isCapped){
		// add the list of identified features to the featureGrid
		var i, feature, idResult, store = this._featureGrid.store, uid, lid, fname, fnames, fnamedelimiters, lalias, compoundFeatureName;
		...
		for (i = 0; i < idResults.length; i++) {
			...
			var displayName = '';
			if (lalias !== ' ') {		
				//Layer alias of a single space means don't show the layer name.		
				displayName += (lalias || idResult.layerName) + ': ';
			}
			if (compoundFeatureName !== '') {
				displayName += compoundFeatureName;
			}
			else {
				if (feature.attributes[fname || idResult.displayFieldName] === 'Null') {
					displayName += 'unknown';
				}
				else {
					displayName += feature.attributes[fname || idResult.displayFieldName] || "";
				}
			}
			...
			store.newItem({
				...
				feature_name: dojo.string.substitute(this._featureRowTemplate, {
					value: compoundFeatureName || (feature.attributes[fname || idResult.displayFieldName] == 'Null' ? 'unknown' : feature.attributes[fname || idResult.displayFieldName]) || "",
					layerName: lalias || idResult.layerName,
		...
		dojo.byId(this.id + "-label").innerHTML = 
			"Identified Features (" +			
			this._featureGrid.rowCount + 
			(isCapped ? ', <small>results limited</small>' : '') + 
			")";
	
		// Show the widget at the clicked location if all IdentifyTasks are complete.
		if (this._identifyTasksRunning == 0) {
			
			if (this._featureGrid.rowCount === 0) {
				//If no records were returned by any of the IdentifyTasks, remove the click graphic from the map and return.
				this._clearGraphics();
				return;
			}			
	...
	_gridRowClickHandler: function(evt){
		var item = evt.grid.getItem(evt.rowIndex), graphic = evt.grid.getItem(evt.rowIndex).ref[0], attr = graphic.attributes, dispAttr = graphic._displayAttributes, aliases = graphic._fieldAliases, urls = graphic._fieldUrls, url;
			...
			for (i = 0; i < len; i++) {
				...
				if (urls[field]) {	
					if (urls[field].urlField != null) {
						if (attr[urls[field].urlField] != 'Null')
					...
					if (urls[field].linkText != null) 
						...
						linkText = attr[field] == 'Null' ? '' : attr[field];
					if (url)
		...
		//Resize the infoGrid to make sure it fits in the space underneath the infoGridHeader. 		
		this._infoGrid.resize({
			h: dojo.position(this._infoPane.domNode).h - dojo.position(this._infoGridHeaderPane.domNode).h
		});
		this._infoGrid.update();
	...
	_onMouseOverQueryComplete: function(featureSet){
		...
		esri.show(this._focusSpan = dojo.byId(this.id + "-flink-" + this._uid));
				...
				for (var j = 0; j < len; j++) {
					...
					if (urls[field]) {
						...
						if (urls[field].linkText != null) 
							...
							linkText = attr[field] == 'Null' ? 'unknown' : attr[field];
						if (url) 
							theValue = '<a href="' + url + '" onClick="window.open(\'' + url + '\'); return false;" target="_blank">' + linkText + '</a>';
						else 
							theValue = linkText;
	...
	_getTaskInfo: function(service){
		var theService = service.service, url = service.service.url, name = service.name, segs = url.replace(/[\/]+$/, "").split("/"), name = name || segs[segs.length - 2].replace(/_/g, " "), chkId = this.id + "-opt-layers-" + (service.id || ++this._uid1);
		return {
			service: theService,
			url: url,
			name: name,
			...
			layerOption: service.layerOption || esri.tasks.IdentifyParameters.LAYER_OPTION_VISIBLE,
	...
	_focusButtonClicked: function(evt, geometry){
		...
		if (geometry.type === "point") {
			...
			if ((featureExtent.spatialReference.wkid === 3857 || featureExtent.spatialReference.wkid === 102100) && 
				...
				var outSR = new esri.SpatialReference({wkt: "PROJCS[\"WGS_1984_Web_Mercator_Auxiliary_Sphere\",GEOGCS[\"GCS_WGS_1984\",DATUM[\"D_WGS_1984\",SPHEROID[\"WGS_1984\",6378137.0,298.257223563]],PRIMEM[\"Greenwich\",0.0],UNIT[\"Degree\",0.0174532925199433]],PROJECTION[\"Mercator_Auxiliary_Sphere\"],PARAMETER[\"False_Easting\",0.0],PARAMETER[\"False_Northing\",0.0],PARAMETER[\"Central_Meridian\",180],PARAMETER[\"Standard_Parallel_1\",0.0],PARAMETER[\"Auxiliary_Sphere_Type\",0.0],UNIT[\"Meter\",1.0]]"});
				
				//Convert the geometry to 180-centered Web Mercator by adding/subtracting a constant from all x coords
				var newGeom;
				if (geometry.type === 'polyline') {
					newGeom = new esri.geometry.Polyline(outSR);
					dojo.forEach(geometry.paths, function(path){
						var newPoints = [];
						for (i = 0; i < path.length; i++) {
							var x = path[i][0] - 20037507.067161795;
							if (x < -20037507.067161795) 
								x += 40075014.13432359;
							newPoints[i] = esri.geometry.Point(x, path[i][1], outSR);
						}
						newGeom.addPath(newPoints);
					}, this);
				} 
				else if (geometry.type === 'polygon') {
					newGeom = new esri.geometry.Polygon(outSR);
					dojo.forEach(geometry.rings, function(ring){
						var newPoints = [];
						for (i = 0; i < ring.length; i++) {
							var x = ring[i][0] - 20037507.067161795;
							if (x < -20037507.067161795) 
								x += 40075014.13432359;
							newPoints[i] = esri.geometry.Point(x, ring[i][1], outSR);
						}
						newGeom.addRing(newPoints);
					}, this);
				} 
				else { //multipoint
					newGeom = new esri.geometry.Multipoint(outSR);
					dojo.forEach(geometry.points, function(point){
						var x = point - 20037507.067161795;
						if (x < -20037507.067161795) 
							x += 40075014.13432359;						
						newGeom.addPoint(esri.geometry.Point(x, point[1], outSR));
					}, this);
				}
				...
				featureExtent = new esri.geometry.Extent(newExtent.xmin - 20037507.067161795, newExtent.ymin, newExtent.xmax - 20037507.067161795, newExtent.ymax, new esri.SpatialReference({
					wkid: 3857
				}));
			}			
